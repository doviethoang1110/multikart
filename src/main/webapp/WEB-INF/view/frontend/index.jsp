...
<section class="p-0">
    <div class="slide-1 home-slider">
        <c:if test="${not empty listRun}">
            <c:forEach items="${listRun}" var="b">
                <div>
                    <div class="home  text-center">
                        <img src="<c:url value="/resources/images/home-banner/${b.image}"/> " alt="" class="bg-img blur-up lazyload">
                        <div class="container">
                            <div class="row">
                                <div class="col">
                                    <div class="slider-contain">
                                        <div>
                                            <h4>${b.content}</h4>
                                            <h1>${b.title}</h1>
                                            <a href="${b.links}" class="btn btn-solid">shop now</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <div class="container">
        <div class="row partition2">
            <c:if test="${not empty listAdvert}">
                <c:forEach items="${listAdvert}" var="b">
                    <div class="col-md-6">
                        <a href="${b.links}">
                            <div class="collection-banner p-right text-center">
                                <div class="img-part">
                                    <img src="<c:url value="/resources/images/home-banner/${b.image}"/> " class="img-fluid blur-up lazyload bg-img"
                                         alt="">
                                </div>
...
<c:if test="${not empty seo}">
    <section class="p-0">
        <div class="full-banner parallax text-center p-left">
            <img src="<c:url value="/resources/images/home-banner/${seo.image}"/> " alt="" class="bg-img blur-up lazyload">
            <div class="container">
                <div class="row">
                    <div class="col">
                ...
                <div class="theme-tab">
                    ...
                    <div class="tab-content-cls">
                        <div id="tab-4" class="tab-content active default">
                            <div class="no-slider row" id="listNew">

                            </div>
                        </div>
                        <div id="tab-5" class="tab-content">
                            <div class="no-slider row" id="listSpecial">
                            </div>
                        </div>
                        <div id="tab-6" class="tab-content">
                            <div class="no-slider row" id="listSales">
                ...
                <div class="slide-3 no-arrow">
                    <c:if test="${not empty listBlog}">
                        <c:forEach items="${listBlog}" var="blog">
                            <div class="col-md-12">
                                <a href="${pageContext.request.contextPath}/bai-viet/${blog.slug}">
                                    <div class="classic-effect">
                                        <div>
                                            <img src="${pageContext.request.contextPath}/resources/images/blog/${blog.image}" class="img-fluid blur-up lazyload bg-img"
                                                 alt="">
                                        </div>
                                        <span></span>
                                    </div>
                                ...
                                <div class="blog-details">
                                    <h4><fmt:formatDate value="${blog.createdAt}"></fmt:formatDate></h4>
                                    <a href="${pageContext.request.contextPath}/bai-viet/${blog.slug}">
                                        <p>${blog.title}</p>
                                    </a>
                                    <hr class="style1">
                    ...
                    <c:forEach items="${sessionScope.listBrand}" var="b">
                        <div>
                            <div class="logo-block">
                                <a href="#"><img src="<c:url value="/resources/images/logos"/>/${b.image}" alt=""></a>
                            </div>
                        </div>
                    </c:forEach>
                </div>
...
<script>
    $(document).ready( function () {
        ...
        print("listSpecial",2);
        print("listSales",3);
        function print(key,index){
            let result = func.filter(r=> r.priority===index).limit(8);
            result.forEach(f=>{
                let name = "";
                let value = "";
                switch (parseInt(f.priority)) {
                    case 1:
                        name = "new";
                        value = "grey";
                        break;
  ...
  top: 7px;
  left: 7px;
  z-index: 1;">`+name+`</span> <span class="lable4">\${f.discount > 0 ? 'on sale' : ''}\</span></div>
                                        <div class="front">
                                            <a href="`+ctx+`/\${f.categorySlug}/\${f.slug}\"><img
                                                src="`+ctx+'/resources/images/pro3/'+f.image+`" class="img-fluid blur-up lazyload bg-img" alt=""></a>
                                        </div>
                                    ...
                                    <div class="product-detail">
                                        ...
                                        <a href="`+ctx+`/\${f.categorySlug}/\${f.slug}\">
                                            <h6>\${f.name}\</h6>
                                        </a>
                                        <h4>
                                            `+new Intl.NumberFormat('en-US',{ style: 'currency', currency: symbol.code }).format(Math.round(parseFloat(symbol.rate !== 1 ? f.from/symbol.rate/1000 : f.from/symbol.rate)))+`-
                                            `+new Intl.NumberFormat('en-US',{ style: 'currency', currency: symbol.code }).format(Math.round(parseFloat(symbol.rate !== 1 ? f.to/symbol.rate/1000 : f.to/symbol.rate)))+`
                                        </h4>
                                    </div>
                            </div>
                        `);
